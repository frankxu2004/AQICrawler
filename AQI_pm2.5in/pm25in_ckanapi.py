...
try:
    ...
    for city in res_all:
        for station in city["stations"]:
            ...
            item["longitude"]=city["location"]["lng"]
            item["latitude"]=city["location"]["lat"]
            item["datetime"]=datetime.datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S")
            item["PM2_5"]=station["data"]["PM2_5"]
            item["CO"]=station["data"]["CO"]
            item["PM10"]=station["data"]["PM10"]
            item["O3"]=station["data"]["O3"]
            item["SO2"]=station["data"]["SO2"]
            item["NO2"]=station["data"]["NO2"]
            ckanPostData.append(item)
    ckanapi.datastore_upsert(resource_id, ckanPostData, api_key)
